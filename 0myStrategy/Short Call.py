...
def short_call_with_fees(
    ...
    """
    محاسبه بازده استراتژی Short Call با احتساب کارمزدها و وجه تضمین

    منطق Short Call:
    - فروشنده حق خرید را به خریدار واگذار کرده و پریمیوم دریافت می‌کند.
    - سود زمانی محقق می‌شود که قیمت سهم ثابت بماند یا کاهش یابد (ITM نشود).
    - حداکثر سود = پریمیوم دریافتی کسر کارمزدها
    - حداکثر زیان = نامحدود (اگر قیمت سهم رشد شدیدی کند)
    """
    ...
    # ========== 3. جریان نقدی اولیه و سرمایه درگیر ==========
    # سود دریافتی اولیه پس از کسر کارمزد
    initial_cash_flow = premium_total + entry_fee
    capital_at_risk = margin_required  # سرمایه درگیر برای محاسبه ROI

    # ========== 4. ارزش ذاتی در سررسید (بدهی فروشنده) ==========
    # Call: اگر stock_price > strike_price باشد، فروشنده باید تفاوت را جبران کند
    intrinsic_liability = max(0, stock_price - strike_price) * contract_size

    # ========== 5. کارمزد اعمال (فقط اگر ITM باشد) ==========
    exercise_fee = 0
    ...
    # ========== 7. بازده درصدی نسبت به سرمایه درگیر ==========
    profit_percent = (
        round((net_profit / capital_at_risk) * 100,
              2) if capital_at_risk != 0 else 0
    )
    ...
    # ========== 8. نقطه سربه‌سر ==========
    # در Short Call: break_even = strike + (premium دریافتی خالص به ازای هر سهم)
    total_credit_per_share = premium_call - (abs(entry_fee) / contract_size)
    if stock_price > strike_price:
    ...
    # هرچه قیمت پایه فعلی پایین‌تر از نقطه سربه‌سر باشد، حاشیه امنیت بیشتر است
    ...
    # ========== 10. درصد OTM بودن موقعیت ==========
    # در Call: اگر stock_price < strike_price باشد یعنی OTM است (امنیتی‌تر برای فروشنده)
    if stock_price > 0 and strike_price > 0:
        max_loss_price_percent = round(
            ((strike_price - stock_price) / stock_price) * 100, 2
        )
    else:
    ...
    if stock_price < strike_price:
        ...
        if price_difference > 0:
            total_premium_income = premium_call * contract_size
            risk_percent = round(
...
def load_and_filter_data():
    """بارگذاری داده‌ها از TSE و فیلتر کردن گزینه‌های اختیار خرید (CALL)"""
...
def run_short_call_strategy(df_options, max_break_even_percent=12):
    """اجرای استراتژی Short Call روی داده‌های فیلتر شده."""
    results_fee = []
    ...
    for underlying_symbol, group in df_options.groupby('UnderlyingTicker'):
        ...
        for _, item in group.iterrows():
            ...
            # اولویت با BidPrice برای فروشنده اختیار است
            ...
            results_fee.append({
                ...
                'max_loss_price_percent_scale': max_loss_price_percent_scale,
    ...
    # فیلتر و مرتب‌سازی (نزولی بر اساس سود ماهانه و حاشیه امنیت)
    ...
    result_df_filtered = result_df_filtered.sort_values(
        ['monthly_return_%', 'break_even_percent'], ascending=[False, False]
    ).reset_index(drop=True)

    return result_df_filtered


def save_results_to_excel(result_df, filename="result_short_call.xlsx"):
    """ذخیره نتایج در فایل اکسل با استایل‌بندی حرفه‌ای."""
    header_font = Font(name='Segoe UI', size=11, bold=True, color='FFFFFF')
    header_fill = PatternFill(
        start_color='4C2882', end_color='4C2882', fill_type='solid'  # بنفش
    )
    alignment = Alignment(
    ...
    with pd.ExcelWriter(filepath, engine='openpyxl') as writer:
        result_df_renamed.to_excel(
            writer, sheet_name='short_call', index=False)
        worksheet = writer.sheets['short_call']
...
def main():
    """تابع اصلی اجرای استراتژی Short Call"""
    try:
        ...
        results = run_short_call_strategy(
