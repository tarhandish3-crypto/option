...
def short_put_with_fees(
    ...
    profit_percent = (
        round((net_profit / capital_at_risk) * 100, 2) if capital_at_risk != 0 else 0)
    ...
    # ========== 10. درصد فاصله قیمت پایه تا strike (میزان OTM بودن) ==========
    if stock_price > 0 and strike_price > 0:
        max_loss_price_percent = round(
            ((stock_price - strike_price) / stock_price) * 100, 2)
    else:
...
def load_and_filter_data():
    ...
    filter_option = df_final[
        ...
        & (df_final['Type'].apply(lambda x: x.name == 'PUT'))].copy()
...
def run_short_put_strategy(df_options, max_break_even_percent=12):
    ...
    for underlying_symbol, group in df_options.groupby('UnderlyingTicker'):
        ...
        for _, item in group.iterrows():
            ...
            # اصلاح شرط اسکیل زمانی: فقط برای گزینه‌های OTM/امن اسکیل زمانی اعمال می‌شود
            if stock_price <= strike_price:
...
def save_results_to_excel(result_df, filename="result_short_put.xlsx"):
    ...
    header_fill = PatternFill(
        start_color='7B2E2E', end_color='7B2E2E', fill_type='solid'
    )
    alignment = Alignment(
    ...
    with pd.ExcelWriter(filepath, engine='openpyxl') as writer:
        result_df_renamed.to_excel(writer, sheet_name='short_put', index=False)
        ...
        # ارجاع دقیق به نام‌های جدید ستون‌ها برای هایلایت اکسل
        highlight_columns = [
