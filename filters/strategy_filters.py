...
def apply_strategy_filter(opp: Opportunity, user_conditions: Dict[str, Any] = None) -> bool:
    ...
    # Bullish Strategies
    # =====================================================
    if "long_call" in name:
        threshold = -7.0 * dte_factor
        indices = pct_changes >= threshold
        return np.all(returns[indices] > 2)

    elif "short_put" in name:
        threshold = -10.0 * dte_factor
        indices = pct_changes >= threshold
        return np.all(returns[indices] >= 2)

    elif "covered_call" in name:
    ...
    # Bearish Strategies
    # =====================================================
    elif "short_call" in name:
        threshold = 10.0 * dte_factor
        indices = pct_changes >= threshold
        return np.all(returns[indices] > 2)

    elif "long_put" in name:
        threshold = 7.0 * dte_factor
        indices = pct_changes >= threshold
        return np.all(returns[indices] >= 2)
    ...
    elif "long_guts" in name:
        neg = -10.0 * dte_factor
        pos = 10.0 * dte_factor
        indices = (pct_changes <= neg) | (pct_changes >= pos)
        return np.all(returns[indices] > 2)
