    ...
    def _calculate_required_margin(legs: List[LegDefinition], underlying_price: float) -> float:
        ...
        try:
            ...
            if hasattr(MarginCalculator, "calculate_strategy_margin"):
                result = MarginCalculator.calculate_strategy_margin(legs, underlying_price)
                # MarginResult object برمی‌گردد، required_margin را استخراج می‌کنیم
                if hasattr(result, "required_margin"):
                    return float(result.required_margin)
                return float(result)
