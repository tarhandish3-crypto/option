...
from strategies.matching.fast_filter import build_pattern_signature, passes_type_filter
...
class FourLegGenerator(BaseGenerator):
    ...
    def __init__(self, strategy_def: StrategyDefinition):
        ...
        # امضای استراتژی برای فیلتر سریع — یک بار محاسبه، بارها استفاده
        self._pattern_sig = build_pattern_signature(strategy_def.patterns)
    ...
    def generate(
        ...
        for candidate in candidate_iterables:
            ...
            # ✅ فیلتر سریع: قبل از PatternMatcher بررسی تعداد Call/Put
            # این فیلتر ۸۰٪+ ترکیب‌های ناسازگار را بدون ساخت LegDefinition رد می‌کند
            if not passes_type_filter(candidate_contracts, self._pattern_sig):
                continue
