...
class DataManager:
    ...
    def get_market_snapshot(self, force_refresh: bool = False, calc_advanced: bool = True) -> MarketSnapshot:
        ...
        Args:
            force_refresh: نادیده گرفتن کش و دانلود مجدد
            calc_advanced: محاسبه BSM، Greeks و IV
            
        Returns:
            MarketSnapshot آماده و ایندکس‌شده برای موتور اسکنر
        """
        ...
        if self.use_cache and not force_refresh:
            ...
            if snapshot_cached is not None and snapshot_cached.option_contracts:
                logger.info(f"Serving MarketSnapshot from memory/disk cache: {len(snapshot_cached.option_contracts)} contracts")
        ...
        for source_name, download_func in download_sources:
            try:
                ...
                if df_raw is not None and not df_raw.empty:
                    logger.info(f"Successfully fetched from {source_name}: {len(df_raw)} records")
                    break
        ...
        if df_raw.empty:
            if self.use_cache:
                logger.warning("All live sources failed. Trying expired cache as fallback...")
                snapshot_expired = self._load_cache(ignore_ttl=True)
                if snapshot_expired is not None and snapshot_expired.option_contracts:
                    logger.warning(f"Using expired cache fallback: {len(snapshot_expired.option_contracts)} contracts")
        ...
        if df_raw.empty:
            raise RuntimeError("Critical Error: All data sources and fallbacks failed to provide market data.")
    ...
    def _process_and_convert(self, df: pd.DataFrame, calc_advanced: bool) -> MarketSnapshot:
        ...
        if calc_advanced:
            logger.info("Calculating advanced metrics (BSM, Greeks, IV)...")
            rf_rate = FinancialCalculator.get_risk_free_rate()
            df = FinancialCalculator.calculate_greeks_vectorized(df, rf_rate)
            logger.info(f"Advanced metrics calculated for {len(df)} records")
        ...
        try:
            ...
            if not ignore_ttl:
                ...
                if age > self.ttl_seconds:
                    logger.debug(f"Disk cache expired (age: {age:.0f}s > {self.ttl_seconds}s)")
                    return None
...
def Options(bsm_greeks: bool = False, use_cache: bool = True, force_refresh: bool = False) -> pd.DataFrame:
    ...
    """
    snapshot = get_market_snapshot(use_cache=use_cache, calc_advanced=bsm_greeks, force_refresh=force_refresh)
